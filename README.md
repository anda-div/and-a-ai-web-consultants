...
案件が増えれば、CLI AI のセッションも同時に複数動きます。そのとき
**共通部品のリポジトリは1つしかない**ため、1つのセッションがブランチを
切り替えると他の案件が影響を受けます。

実際に、1日のあいだに3つのセッションが同じリポジトリを触り、
「共通部品を直したまま放置」「`shared/` を見ずに『無い』と結論」
「`git switch main` で自分の変更が消えた」「同じファイルを触るPRが2本」
の4件が起きました。**4件すべて、隣のセッションに一言聞けば済んだこと**です。

Claude Code には他のセッションを一覧して直接やり取りする機能があります。
使い分け（**事実確認は直接、方針の決定は利用者を経由**）と最低限の約束を
[shared/MULTI_SESSION.md](shared/MULTI_SESSION.md) にまとめています。
