...
### 絞り込みは「イベント単位」で評価される ── ここが最大の壁

`runReport` の `dimensionFilter` は、**1つのイベントが条件を同時に満たすか**で判定する。
セッション単位ではない。したがって次は**必ず0になる。**

```python
# 誤り。購入イベントに商品ページのタイトルは付いていない
G.f_and(G.event_filter("purchase"), G.f_contains("pageTitle", "ある商品"))
```

「ある行動をしたセッションの、購入や収益」を取りたいときに必ず踏む。
**合計が0になるので気づきやすいが、原因が分かりにくい。**
GAS のコメントに警告が書かれていた案件でも、読んでいた人が同じ穴に落ちた。

取り方は、欲しいものによって変わる。

| 欲しいもの | 取り方 |
| --- | --- |
| **人数・セッション数** | ファネルの**ステップ1に条件を置く**（2段にする）。`api.funnel_rows()` |
| **収益（商品属性で表せる）** | **item スコープだけで絞る。** `itemRevenue` を `itemBrand` / `itemName` で絞り、**`eventName` を混ぜない**（混ぜると「dimensions and metrics are incompatible」になる） |
| **収益（行動でしか表せない）** | **APIでは取れない。** 管理画面の探索（セッションスコープのセグメント）→ TSV → 集計 |

3行目を認めるのが大事である。`runFunnelReport` の `segments` はセッション単位に
対応しているが、**ファネルが返すのは人数であって収益ではない。**
共通部品も `funnelBreakdown` までで、`segments` は組んでいない。

**「絞り込みを実行した人の売上」は、商品属性では代替できない。**
絞り込みを使わずにその商品を買う人も、絞り込みを使って別の商品を買う人もいる。
母集団が別物なので、item スコープで出した数字を代わりに使ってはいけない。

### 「含む」で絞るときは、エンコードされた形も数える

URLパラメータは**2つの形で記録される**ことがある。
配列を渡すパラメータ（`?p[]=...` の形）で実際に起きた。

| `pageLocation` に含む文字列 | 割合 |
| --- | --- |
| `p%5B%5D`（エンコード形） | 全体の約8割 |
| `p[]`（デコード形） | 全体の約2割 |
| `p`（どちらも拾う） | **全体** |

エンコード形だけで絞ると**2割前後を取りこぼす。** 比率は案件によって変わるので、
**必ず両方を数えて確かめる。** 角括弧や日本語を含むパラメータは、
**記号を含まない部分だけで `CONTAINS` する**のが安全。
管理画面の探索でフィルタを組むときも同じ。
...
セッション数やイベント数は動かないのに、**ユーザー数だけが 0.05% ほどずれる**ことがある。
これは実装の誤りではない。GA4 側がユーザーの推定値をあとから作り直すためである。

実際にそうだと確かめられた案件では、**1枚の表の中で**きれいに分かれた。

| 行の性質 | 結果 |
| --- | --- |
| 流入・直帰・直帰率（セッション系） | 小数点以下まで**完全に一致** |
| 閲覧者数・ファネルの各ステップ（**ユーザー**系） | 0.05% 前後**ずれた** |

同じシート・同じ期間・同じ1回の照合である。絞り込みも期間も共通で、
違うのは指標の性質だけだった。

さらに、期間を3列（前年同月・前月・当月）並べた表では**当月の列だけ**がずれ、
前月と前年同月の列はファネルの値まで含めて完全に一致した。
比較相手は、当月が終わった5日後に作られていた。

つまり、**新しい期間ほどユーザー数は動く**。落ち着いた過去の期間は動かない。

見分け方は次のとおり。

- 差が出た指標が `totalUsers` / `activeUsers` 系だけに寄っていないか
- 同じ表のセッション系・イベント系が一致しているか（一致していれば期間も絞り込みも正しい）
- 期間を並べた表なら、**古い列が一致し、新しい列だけがずれて**いないか

この3つが当てはまるなら、**コードを直して差を消してはいけない。**
差を消せるのは、丸め方を旧側に合わせるなどの「同じ入力から同じ出力を出す」直しだけである。
入力そのものが変わっているものを合わせようとすると、係数の押し込みになる。

記録に残す言い方の例。

> ユーザー数系の指標に 0.05% 前後の差が出ています。同じ表のセッション系の指標は
> 全期間・全日付で一致しており、期間・絞り込みの移植が正しいことは確認できています。
> 差は GA4 がユーザー数の推定を後から作り直すことによるもので、
> 落ち着いた過去の期間の列は完全に一致しています。

### 標本で答えられている問い合わせは、日をまたぐと一致しない

ユーザー数のずれ（上）とは別に、**セッション数まで食い違う**ことがある。
条件の複雑な問い合わせに対して、GA4 が全数ではなく**標本**で答えている場合である。

見分け方は簡単で、応答が教えてくれる。

```python
res = api.client.run_report(req)           # 案件側は api.report(...) 越しでよい
sm = list(res.metadata.sampling_metadatas or [])
if sm:
    print(sm[0].samples_read_count / sm[0].sampling_space_size)   # 例 0.03
```

ある案件で実際に出た対応。**1つの例外もなく一致した。**

| 問い合わせ | GA4 の扱い | 照合結果 |
| --- | --- | --- |
| 絞り込みなし・当月 | 全数 | 差なし |
| 単一項目の絞り込み・当月 | 全数 | 差なし |
| 入れ子の OR/AND・当月（31日） | 標本 約 40% | 差あり |
| 同じ条件・13か月（396日） | 標本 約 3% | 差あり（数% まで） |

**期間が長いほど標本率が下がり、差が大きくなる。**

決定的な確かめ方は、**同じ問い合わせを同じ日に2回投げること。**
標本は同じ日のうちは引き直されないため、自分の出力どうしは完全に一致する。

```
同じ問い合わせ2回: 日数 396 / 食い違った日 0
```

ここが一致するなら、**コードは決定的である。**動いているのは GA4 の答えのほうで、
比較相手が別の日に作られていれば、別の標本で書かれている。

この場合も**直してはいけない。** 旧方式（GAS など）も同じ API に同じ条件で
問い合わせている以上、同じように標本で答えられている。旧方式をいま実行し直しても、
過去の出力とは同じように食い違う。

記録に残す言い方の例。

> 一部の数値に 0.1〜3% の差が出ています。これは GA4 が、条件の複雑な問い合わせに
> 対して全数ではなく標本で答える仕様によるものです。同じ条件を同じ日に2回
> 問い合わせれば全日程で一致することを確認しており、集計の定義や期間の解釈に
> 誤りがないことは確かめられています。全数で答えが返る問い合わせは、
> 従来の出力と1セルも違いません。

差が減らせるかどうかは、**条件を単純にできるかどうか**で決まる。
単純にすると集計の定義が変わるので、**照合のために定義を変えてはいけない。**
...
月次1回で数百本の問い合わせを投げる案件では、**正しく書けていても途中で 429 で止まる。**

```
Exhausted property tokens for a project per hour.
```

待てば戻る。**失敗として捨てないこと。** 共通部品（`ga4_client.py`）は
残りトークンを応答から読み（`returnPropertyQuota`）、少なくなったら自分から待つ。

枠は用途ごとに分かれている。**先に尽きるのは、いちばん小さい枠。**

| 枠 | 桁 | 効き方 |
| --- | ---: | --- |
| `tokensPerDay` | 十万台 | まず当たらない |
| `tokensPerHour` | 数万 | まず当たらない |
| `tokensPerProjectPerHour` | **数千** | `runReport` はここで止まる |

**割り当てはプロパティごとに違う。** 上は桁の目安なので、自分の案件では
`returnPropertyQuota`（Python なら `return_property_quota=True`）を付けて、
応答の `propertyQuota` を実際に見ること。

**`runFunnelReport` は別枠で、こちらのほうがずっと小さい。**
しかも残りの量が応答に出てこない。ファネルの枠が尽きていても `runReport` 側は
まだ動くので、**ファネルを使わないシートだけ先に進める**と作業が止まらない。

案件側の取得スクリプトは、次の2つを持たせておくとよい。

- シート1枚ごとに結果を残す（途中で落ちても、残りだけを取りに行ける）
- 「ファネルを使うシート／使わないシート」で流し分けられる切り替え

**並列で走らせる本数は控えめにすること。** 2本流すと2倍の速さで枠を食い、
どちらも 429 で止まる。
