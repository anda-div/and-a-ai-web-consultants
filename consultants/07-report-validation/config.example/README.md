...
`../defaults/` に汎用の既定値があります。ここに書かなければ既定が使われ、
書けばキー単位で上書きされます。**書く量は少ないほど良い**設計です。

`_config/` は納品後、利用者のPC上に置かれ、提供者が直接は直せません。
汎用側に置けるものを `_config/` に書くと、改善のたびに個別連絡が必要になります。
...
| `competitor_history.json` | 競合×月×切り口の履歴 | 毎月（競合章を作ったとき） |
