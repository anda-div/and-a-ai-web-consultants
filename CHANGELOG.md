...
- `shared/GTM_WORKSPACE.md` を追加。**GTMのワークスペースを取り違えると、変更を入れたのに反映されない。**しかも、入れた側にも公開した側にも成功して見える。既定のワークスペースで作業しない、着手前にどれに入れるかを確認する、公開を依頼する資料にコンテナIDとワークスペース名を併記する、作業後にワークスペース名と変更数を記録する、の4つ。**変更数は、手で数え直さずGTMの画面が数えた値を写す**（同じタグへの付け替えと停止は作業2回だが、GTMは変更されたエンティティを数えるので1件）。GTMにはワークスペース間で変更を移す機能が無いため、入れ違えると作り直しになる。ルートの `CLAUDE.md` と `AGENTS.md` から参照する。設定変更はどのJOBの手順にも含まれない（JOB 01 は点検専門で、設定変更は「行わないこと」に明記）ため、共通の作法として `shared/` に置いた。
- JOB 03がPtengineのヒートマップに対応。`PTENGINE_CAPTURE.md` と `scripts/ptengine_heatmap_capture.py` / `ptengine_capture_set.py` を追加。Clarityとの違いは2つで、**状態がURLに載らない**ことと、**下敷きが実サイト**であること。期間・デバイス・種別はクライアント側に保存され再読み込みしても残るため、撮る直前に画面から読み戻し、指定と違えば撮らずに止める。記録には指定ではなく読み戻した値（`observedSelection`）を残す。結合・余白落とし・貼り付く要素の判定は `scripts/heatmap_image.py` に集めてClarityと共用し、`clarity_heatmap_capture.py` から重複していた176行を外した。実機で分かった落とし穴を19件文書化。うち3件は最初に誤って結論したもので、経緯ごと残している（トークンを「不要」と判断した／貼り付く要素を画像の統計で測ろうとした／熱レイヤーの高さの食い違いを位置ずれだと書いた）。
- JOB 03のPtengineで、任意の暦月をカレンダーから指定できるようにした。月送りの矢印は中身がSVGで文字を持たず、矢印記号では見つからないが、クラス名（`pt-icon-arrow-left` 等）には向きが入っている。**押し方が期間ボタンと逆で**、期間ボタンは本物のマウス操作、カレンダーの矢印はDOMの `click()` でないと動かない（本物のマウス操作は例外も出さずに素通りする）。あわせて、暦月を「先月」等のプリセットへ読み替えるのをやめた。読み替えは実行した日に依存し、月末の深夜に始めた取得が日付をまたぐと指す月が1つずれるため。遡れるのは実測で約12か月。判断の部分を `scripts/ptengine_dates.py` に切り出し、ブラウザ無しで確かめるテスト16件を追加した。
- CIで動いていなかったJOB 03のテストを `quality.yml` に載せた。`test_heatmap_image.py`（27件）はファイルを追加しただけでワークフローに載せ忘れていた。`test_ptengine_dates.py`（16件）とあわせて毎回動かす。**足しただけで動いていないテストは、無いのと同じ。**
- `.gitignore` にPtengine用ブラウザプロファイル（`.ptengine_profile`）を追加。
